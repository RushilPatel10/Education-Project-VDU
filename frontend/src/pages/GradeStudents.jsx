  ...
  useEffect(() => {
    ...
    const fetchSubmissions = async () => {
      try {
        ...
        const response = await axios.get(`${API_BASE_URL}/api/grades/submissions/${courseId}`, {
          headers: {
  ...
  const handleGradeSubmit = async (assignmentId) => {
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/grades/grade/${assignmentId}`,
        { grade },
        {
          headers: {
            Authorization: `Bearer ${token}`,
              ...
              <tbody>
                {submissions.map((submission) => (
                  <tr key={submission._id}>
                    ...
                    <td><a href={`${API_BASE_URL}/${submission.filePath}`} target="_blank" rel="noopener noreferrer">Download</a></td>
